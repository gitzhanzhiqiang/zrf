<template>
    <div class="app-container calendar-list-container ggg_user">
        ...
        <div class="filter-container">
            <!--<div class="tableheader">当日还款统计</div>-->
            <el-table :data="dataList" v-loading="listLoading" border fit highlight-current-row style="width: 100%" :default-sort="{prop: order, order: dir}" @sort-change="sort">
                ...
                <el-table-column align="center" label="未还款笔数" sortable="custom" prop="unNum">
                    <template scope="scope">
                        {{scope.row.unNum | isNumber}}
                    </template>
                </el-table-column>
                <el-table-column align="center" label="回款率" sortable="custom" prop="returnMoneyRate">
                    <template scope="scope">
                        {{scope.row.returnMoneyRate}}%
                    </template>
                </el-table-column>
            </el-table>
        </div>
...
export default {
    ...
    methods: {
        getTable(isseach) {
            ...
            ajax({
                url: 'PlatformData/nowDayReturnMoneyStatistics',
