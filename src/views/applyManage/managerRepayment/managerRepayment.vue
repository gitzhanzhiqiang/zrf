<template>
	<div class="app-container calendar-list-container ggg_user">
		...
		<el-button type="primary" style="float: right;margin-bottom: 20px" @click="$router.push({path:'stayStillbasket'})">待扣篮</el-button>
		<div class="filter-container">
			<!--<div class="tableheader">店长还款</div>-->
			<el-table :data="dataList" v-loading="listLoading" border fit highlight-current-row style="width: 100%">
				...
				<el-table-column align="center" label="当期状态">
					<template scope="scope">
						<span :class="{'red': item.id == 20}" v-for="item in storage.repaymentStatus1" :key="item.id" v-text="item.label" v-show="item.id == scope.row.repaymentStatus"></span>
					</template>
				</el-table-column>
		...
		<el-dialog title="查看" :visible.sync="dialogVisible" width="30%">
					...
					<table class="table">
						...
						<tr>
							...
							<td>
								<el-row :gutter="0">
									<el-col :span="7">
										<el-form-item label="" prop="repaySource" label-width="0">
											<el-select v-model="alertForm.repaySource" placeholder="请选择" @change="alertForm.bankCardNo = '';bankUserInfo=''">
												<el-option style="color: red;" label="使用店长银行卡 " value="1"></el-option>
												<el-option label="使用用户银行卡 " value="2"></el-option>
											</el-select>
										</el-form-item>
									</el-col>
									<el-col :span="5">
									...
									<el-col :span="6">
										<el-form-item label="" prop="bankCardNo" label-width="0">
											<el-select v-model="alertForm.bankCardNo" placeholder="请选择" @change="bankChange" v-if="alertForm.repaySource == 1">
												<el-option v-for="(item, index) in bankData.cardList" :key="index" :label="item.bankCardNo" :value="item.bankCardNo">
												</el-option>
											</el-select>
											<el-select v-model="alertForm.bankCardNo" placeholder="请选择" @change="bankChange" v-else>
												<el-option v-for="(item, index) in bankData.userCardMes" :key="index" :label="item.bankCardNo" :value="item.bankCardNo">
												</el-option>
											</el-select>
										</el-form-item>
									</el-col>
									<el-col :span="6">
										<el-form-item label="开户行:" label-width="58px">
											<span style="width:100px;">{{bankUserInfo.bankName}}</span>
										</el-form-item>
									</el-col>
								</el-row>
							</td>
						...
						<tr>
							...
							<td>
								<el-form-item label-width="0" prop="periodNum">
									<el-select v-model="alertForm.periodNum" placeholder="请选择" @change="alertFormChangeSelect" v-if="alertForm.type != 40 && alertForm.type != 41 && alertForm.type != 32 && alertForm.type != 42">
						...
						<tr v-if="alertForm.type == 30 || alertForm.type ==40  || alertForm.type ==42">
							<td>建议还款金额</td>
							<td>
								<el-form-item label-width="0">
									<span v-if="alertForm.type == 30">{{moneyData.suggestRepayAmount | formatMoney}}</span>
									<span v-else>{{moneyData.suggestRepayAmount | formatMoney}}~{{moneyData.suggestRepayAmount + moneyData.lateFeeAmount | formatMoney}}</span>
								</el-form-item>
							</td>
						</tr>
						<tr v-if="alertForm.type != 30 && alertForm.type !=40 && alertForm.type !=42">
						...
						<tr v-if="alertForm.type == 32 || alertForm.type == 41 ||  ahead_schedule_status || alertForm.type == 42">
							<td>实际还款金额</td>
							<td>
			...
			<span slot="footer" class="dialog-footer">
                ...
                <el-button type="primary" @click="submitAlert('alertForm')" :loading="loading" v-if="alertForm.type != 40">确 定</el-button>
                <el-button type="primary" @click="bring_forward('alertForm')" v-else>确 定</el-button>
                <el-button type="primary" @click="submitAlert('alertForm', true)"  v-if="alertForm.type != 40" v-show = 'xianshi' :loading="loading">加入待扣篮</el-button>
                 <el-button type="primary" @click="bring_forward('alertForm', true)" v-else>加入待扣篮</el-button>
            </span>
		</el-dialog>
	</div>
</template>
<script>
	...
	export default {
		...
		methods: {
			...
			//点击银行卡
			bankChange(idCard) {
				let name = this.alertForm.repaySource == 1 ? 'cardList' : 'userCardMes';
				console.log(name)
				this.bankData[name].map((item) => {
			...
			alertSelectChange(value, isFirst) {
				...
				console.log(value)
				if(!isFirst) {
					if(this.alertForm.type == 32 || this.alertForm.type == 41 || this.alertForm.type == 42) {
						this.xianshi = false
					} else {
						this.xianshi = true
					}
				...
				ajax({
					...
					.then(response => {
						if(response.code === 200) {
							...
							this.bankData.cardList = data.cardList ? data.cardList : []; //店长银行卡
							this.bankData.userCardMes = data.userCardMes ? data.userCardMes : []; //用户银行卡
			...
			//弹框提交
			submitAlert(formName, isShop) {
				console.log(this.alertForm.type)
				console.log(isShop)
				console.log(this.rowData.name)
				console.log(this.bankUserInfo.idCardName)
				this.$refs[formName].validate((valid) => {
					if(valid) {
						...
						let realRepayAmount = '';
						if(this.alertForm.type == 32 || this.alertForm.type == 41 || this.alertForm.type == 40 || this.alertForm.type == 42) {
							realRepayAmount =Math.ceil(this.alertForm.realRepayAmount * 100)
						} else {
						...
						console.log(realRepayAmount)
						ajax({
								url: 'shopOwnerRepay/subOrder',
								optionParams: {
									sbxq: sbxq,
									type: this.alertForm.type,
									...
									repaySource: this.alertForm.repaySource, //银行卡是店长还是用户
									...
									suggestRepayAmount: this.moneyData.suggestRepayAmount, //建议还款金额
									dueLateFeeAmount: this.moneyData.lateFeeAmount, //罚息
									loanName: this.rowData.name, //借款人
									bankUserName: this.bankUserInfo.idCardName, //银行卡开户人
									shopId: this.rowData.shopId,
								}
							}).post()
							.then(response => {
			...
			bring_forward(formName, isShop) {
				...
				this.$refs[formName].validate((valid) => {
					if(valid) {
						...
						console.log(realRepayAmount)
						this.$confirm('是否将借款人（' + this.rowData.name + '）发起提前申请， 请确保卡内余额充足', '提示', {
