    ...
    <div class="app-container calendar-list-container ggg_user">
        ...
        <el-form :inline="true" :model="params" ref="params" class="demo-form-inline" @keyup.enter.native="getTable('isseach')">
            ...
            <el-form-item label="订单状态" prop="orderStatus">
                <el-select v-model="params.orderStatus" placeholder="请选择">
                    <el-option v-for="item in storage.orderStatus" :key="item.id" :label="item.label" :value="item.id"></el-option>
                </el-select>
            </el-form-item>
        ...
        <div class="filter-container" style="overflow:hidden;">
            <div style="float:right;">
                <el-button type="danger" class="iconfont icon-xiazai1" @click="koudao" :disabled='disabled'> 扣款失败导出</el-button>
                <el-button type="success" class="iconfont icon-xiazai1" @click="mingdao"> 明天待还导出</el-button>
            </div>
        </div>
        <div class="filter-container">
            ...
            <el-table :data="dataList" v-loading="listLoading" border fit highlight-current-row style="width: 100%">
               ...
               <el-table-column align="center" label="订单状态" width="90">
                ...
                <el-table-column align="center" label="门店" width="90">
                    <template scope="scope">
                        {{scope.row.shopName}}
                    </template>
                </el-table-column>
                ...
                <el-table-column align="center" label="操作" fixed="right" width="70">
                    <template scope="scope">
                        <el-button type="text" size="small" @click="$router.push({path:'applyMessageDetail', query:{data: JSON.stringify(scope.row)}})">查看</el-button>
                    </template>
                </el-table-column>
            </el-table>
        </div>
...
export default {
    data() {
        return {
            ...
            params: { //搜索条件
                type: 2, //必传 type 查询类型 1：行政查询 2：初审查询 *3：终审查询* 4：待合同上传界面查询* 5:财务待放款查询* 6：财务拒绝查询
                pageSize: 10,
                pageIndex: 1,
                userName: null, //用户名字
                orderNum: null, //订单编号
            ...
            dataList: [],
            disabled: true,
            imgxu :require('@res/img/aa.png'),
             imgxua :require('@res/img/aaa.png'),
        }
    },
    computed: {
    },
    ...
    methods: {
        ...
        // 明天代还导出
        mingdao() {
            downloadExecl({ url: 'export/downTomorrow', input: {}, method: 'get' })
        },
        //  扣款失败导出
        koudao() {
            downloadExecl({ url: 'export/downErrorNow', input: {}, method: 'get' })
        },
        getTable(isseach) {
