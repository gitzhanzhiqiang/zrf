	...
	<div class="app-container calendar-list-container ggg_user" v-loading="listLoading">
		<el-tabs type="border-card" @tab-click="handleClick" >
			<el-tab-pane label="核心数据">
				<el-form :rules="rules" ref="alertFrom" label-position="center" label-width="0px" :model="form" class="table_css kernel" :class="{noborder: !isEdit}">
					<!--身份证-->
					...
					<div class="filter-container">
						...
						<div class="filter-container" style="overflow:hidden;">
							<div style="float:right;">
								<el-button type="success" @click="downloadjie" class="iconfont icon-xiazai1"> 结清证明</el-button>
							...
							<tr>
								...
								<td>
									<el-form-item prop="applyAmount">
										<el-input v-model="form.applyAmount" :placeholder="placeholder" :readonly="!isEdit" @keyup.native="inputChange($event, false, 'applyAmount')"></el-input>
									</el-form-item>
								</td>
							</tr>
							<tr>
								...
								<td>
									<el-form-item prop="loanTerm">
										<el-input v-model="form.loanTerm" :placeholder="placeholder" readonly></el-input>
									</el-form-item>
								</td>
							</tr>
							<tr>
							...
							<tr>
								...
								<td>
									<el-form-item prop="eduBg">
										...
										<span class="el-input" v-for="item in eduBg" v-text="item.name" v-show="item.value == form.eduBg" v-else></span>
									</el-form-item>
								</td>
								<td>婚姻状况</td>
								<td>
									<el-form-item prop="marriageStatus">
										...
										<span class="el-input" v-for="item in marriageStatus" v-text="item.name" v-show="item.value == form.marriageStatus" v-else></span>
									</el-form-item>
								</td>
							</tr>
							<tr>
								...
								<td>
									<el-form-item prop="houseStatus">
										...
										<span class="el-input" v-for="item in houseStatus" v-text="item.name" v-show="item.value == form.houseStatus" v-else></span>
									</el-form-item>
								</td>
								<td>子女数目</td>
								<td>
							...
							<tr>
								...
								<td>
									<el-form-item prop="residenceAddr">
										<el-input type="textarea" :rows="3" :placeholder="placeholder" v-model="form.residenceAddr" :readonly="!isEdit"></el-input>
									</el-form-item>
								</td>
								<td>现在住址</td>
								<td>
									<el-form-item prop="presentAddr">
										<el-input type="textarea" :rows="3" :placeholder="placeholder" v-model="form.presentAddr" :readonly="!isEdit"></el-input>
									</el-form-item>
								</td>
							</tr>
						</table>
							...
							<tr>
								...
								<td>
									<el-form-item prop="enterpriseType">
										...
										<span class="el-input" v-for="item in enterprise_type" v-text="item.name" v-show="item.value == form.enterpriseType" v-else></span>
									</el-form-item>
								</td>
							</tr>
							<tr>
								...
								<td>
									<el-form-item prop="industry2">
										<el-select v-model="form.industry2" placeholder="请选择" @change="industryChange" v-if="isEdit">
											<el-option :label="item.label" :value="item.id" v-for="item in industry" :key="item.id"></el-option>
										</el-select>
										<span class="el-input" v-for="item in industry" v-text="item.label" v-show="item.id == form.industry2" v-else></span>
									</el-form-item>
								</td>
								<td>单位行业</td>
								<td>
									<el-form-item prop="industry">
										<el-select v-model="form.industry" placeholder="请选择" v-if="isEdit">
											<el-option :label="item.label" :value="item.id*1" v-for="item in industry2" :key="item.id"></el-option>
										</el-select>
										<span class="el-input" v-for="item in industry2" v-text="item.label" v-show="item.id == form.industry" v-else></span>
									</el-form-item>
								</td>
							</tr>
							<tr>
							...
							<tr>
								...
								<td>
									<el-form-item prop="monthIncome">
										<el-input :placeholder="placeholder" v-model="form.monthIncome" @keyup.native="inputChange($event, true, 'monthIncome')" :readonly="!isEdit"></el-input>
									</el-form-item>
								</td>
							</tr>
							<tr>
					...
					<!--建议x循环-->
					<div class="filter-container noborder" v-show="AuditData.spStatus != 1 && AuditData.spStatus != 7" v-for="(item,index) in AuditData.spHistory">
					...
					<!--建议-->
					<div class="filter-container" v-show="AuditData.spStatus == 1 || AuditData.spStatus == 7">
						<table style="" class="table">
							<tr>
								<td>初审意见</td>
								<td>
									<p style="width:200px;display:inline-block;">
										<el-form-item>
											<el-select v-model="AuditForm.status" placeholder="请选择">
												<el-option label="建议通过" value="1"></el-option>
												<el-option label="建议拒绝" value="2"></el-option>
											</el-select>
										</el-form-item>
									</p>
									<p style="width:200px;display:inline-block;">
							...
							<tr>
								...
								<td colspan="5">
									<el-form-item prop="msg">
										<el-input type="textarea" :rows="3" :placeholder="placeholder" v-model="AuditForm.msg"></el-input>
									</el-form-item>
								</td>
							</tr>
						</table>
			...
			<el-tab-pane label="影像资料" name="second1">
				<div>
				...
				<div class="filter-container">
					...
					<div class="filter-container">
						<el-button type="primary" icon="upload2" @click="isAlert()" v-show="imgtype != 8 && imgtype !=3 && rowData.loanStatus != 14">上传</el-button>
						<div class="upLoadingAlert1">
							<ul class="el-upload-list el-upload-list--picture">
								<li class="el-upload-list__item is-success" v-for="(item, index) in imgData">
									<img :src="imgSrcBase + item.consumeImg + '?x-oss-process=style/zrf'" class="el-upload-list__item-thumbnail" alt="" @click="magnifyImg(imgData, index)">
									...
									<i class="el-icon-close" @click="deleteImg(item)" v-if="rowData.orderStatus < 30"></i>
								</li>
							</ul>
						</div>
					</div>
				...
				<div class="filter-container">
					<table class="table">
						...
						<tr>
							...
							<td>申请产品</td>
							<td v-text="detailplanData.productName"></td>
							<td>借款期限</td>
			...
			<el-tab-pane name="" label="运营商报告">
				<el-tabs type="card" @tab-click="operatorreportfu">
								...
								<div class="filter-container">
									<table class="table table_iphone">
										...
										<tr v-for="item in operatorreporta.main_service">
											...
											<td><span style='background: #d9edf7;margin-right: 5px;' v-for="item in item.service_details">{{item.interact_mth}}&nbsp;{{item.interact_cnt}}</span></td>
										</tr>
									</table>
								</div>
							</div>
					...
					<el-tab-pane label="通话详单、网页登录截图">
						<el-form ref="iphone_Form" label-position="center" :model="iphone_Form" label-width="100px" style="width:400px;">
							<el-form-item label="通话详单" prop="nickname">
								<!--<el-input v-model="iphone_Form.nickname"></el-input>-->
								<el-button type="primary">上传</el-button>
							</el-form-item>
							<el-form-item label="其他" prop="userName">
								<!--<el-input v-model="iphone_Form.userName"></el-input>-->
								<el-button type="primary">上传</el-button>
							</el-form-item>
							<el-form-item label="网站登录截图" prop="phone">
								<!--<el-input v-model="iphone_Form.phone"></el-input>-->
								<el-button type="primary">上传</el-button>
							</el-form-item>
							<el-form-item>
			...
			<el-tab-pane label="手动获取运营商">
				<el-form ref="iphoneForm" label-position="center" :model="params" label-width="110px" style="width:400px;" v-show="iphoneOperatoriSshow == 1">
					<el-form-item type="number" label="手机服务密码" :rules="[{required: true, message: '请输入', trigger: 'blur'}]" prop="iphone">
						<el-input v-model="params.iphone" placeholder="请输入" type="number"></el-input>
					</el-form-item>
					<el-form-item>
						<el-button type="primary" @click="next_Servicecode ('iphoneForm')">下一步</el-button>
					</el-form-item>
				</el-form>
				<el-form ref="iphoneForm1" label-position="center" :model="SMScoede" label-width="110px" style="width:400px;" v-show="iphoneOperatoriSshow == 2">
					<p style="padding:20px;font-size:12px;">本次操作需要短信验证，验证码已发送您的手机
						<span>4tre</span>
					</p>
					<el-form-item type="number" label="短信验证码" :rules="[{required: true, message: '请输入', trigger: 'blur'}]" prop="code">
						<el-input v-model="SMScoede.code" placeholder="请输入" type="number"></el-input>
					</el-form-item>
					<el-form-item>
						<el-button type="primary" @click="next_SMScode ('iphoneForm1')">下一步</el-button>
					</el-form-item>
				</el-form>
				<el-form ref="iphoneForm2" label-position="center" :model="SMScoede1" label-width="110px" style="width:400px;" v-show="iphoneOperatoriSshow == 3">
					<p style="padding:20px;font-size:12px;">本次操作需要短信验证，验证码已发送您的手机
						<span>4tre</span>
					</p>
					<el-form-item type="number" label="短信验证码" :rules="[{required: true, message: '请输入', trigger: 'blur'}]" prop="code">
						<el-input v-model="SMScoede1.code" placeholder="请再次输入" type="number"></el-input>
					</el-form-item>
					<el-form-item>
						<el-button type="primary" @click="bindIphone('iphoneForm2')">绑定运营商</el-button>
					</el-form-item>
				</el-form>
			</el-tab-pane>
		</el-tabs>
...
export default {
	...
	data() {
		...
		return {
			...
			rules: { //验证
				...
				// loanTerm: [{ required: true, message: '请输入', trigger: 'blur' }],
				consumeType: [{ type: 'number', required: true, message: '请选择', trigger: 'change' }],
				agreementAmount: [{ required: true, message: '请输入', trigger: 'blur' }],
				realPayAmout: [{ required: true, message: '请输入', trigger: 'blur' }],
				eduBg: [{ type: 'number', required: true, message: '请选择', trigger: 'change' }],
				marriageStatus: [{ type: 'number', required: true, message: '请选择', trigger: 'change' }],
				houseStatus: [{ type: 'number', required: true, message: '请选择', trigger: 'change' }],
				childNumber: [{ type: 'number', required: true, message: '请选择', trigger: 'change' }],
				...
				enterpriseType: [{ type: 'number', required: true, message: '请选择', trigger: 'change' }],
				industry2: [{ required: true, message: '请选择', trigger: 'change' }],
				industry: [{ type: 'number', required: true, message: '请选择', trigger: 'change' }],
				...
				shopId: [{ type: 'number', required: true, message: '请选择', trigger: 'change' }],
				adminId: [{ type: 'number', required: true, message: '请选择', trigger: 'change' }],
			...
			writeList: [],// 白名单列表
			blankList3: {}, //黑名单3
			iphoneOperatoriSshow: 1, //手机运营商几个提交框显示
			iphone_Form: {}, //通话详单
			params: {
				iphone: ''
			}, //运营商提交
	...
	created() {
		...
		this.rowData = data;
		console.log(this.rowData.loanStatus);
		this.getDetail();
	...
	methods: {
		...
		//下载手 结清证明
		downloadjie() {
			window.open(baseURL() + 'detailsdata/jqzm?orderNum=' + this.rowData.orderNum)
		},
		// 强制正整数
			...
			ajax({
				...
				.then(response => {
					if (response.code === 200) {
						...
						//希望输入金额
						this.form.applyAmount = this.form.applyAmount / 100 + '';
					} else {
			...
			ajax({
				...
				.then(response => {
					if (response.code === 200) {
						...
						if (this.AuditData.spStatus == 7 || this.AuditData.spStatus == 1) {
							this.isEdit = true;
						} else {
							this.isEdit = false;
						}
		...
		submitForm(formName) {
			...
			this.$refs[formName].validate((valid) => {
				...
				subMitform.applyAmount = subMitform.applyAmount * 1;
				if (valid) {
					this.loading = true; //二次提交按钮设置
					let reasonsRejection = this.AuditForm.status == 2 ? this.AuditForm.reasonsRejection : ''; //拒绝时才有拒绝原因，其他为空
					ajax({
					...
					if (response.code === 200) {
						...
						this.industry.map((value) => {
							...
							value.children.map((item) => {
								console.log('aaaaaaa' + this.form.industry)
		...
		getDetailplan() {
			ajax({
				...
				.then(response => {
					if (response.code === 200) {
						console.log(response.data.orderRepayPlanVo)
			...
			ajax({
				...
				.then(response => {
					if (response.code === 200) {
						...
						console.log(this.operatorreporta.contact_list)
					} else {
						this.$message.warning(response.message)
					}
					this.listLoading = false;
        ...
        blacklistTab(tab, event) {
            ...
            if (tab.index == 1) {
                this.getWriteList(3); //获取关联关系
            }
			if (tab.index == 2) {
                this.getWriteList2(); //获取关联关系
            }
        },
		  //获取白名单
        getWriteList(value) {
		...
		//服务码下一步
		next_Servicecode(formName) {
			this.$refs[formName].validate((valid) => {
				if (valid) {
					this.iphoneOperatoriSshow = 2;
		...
		//短信验证码下一步
		next_SMScode(formName) {
			this.$refs[formName].validate((valid) => {
				if (valid) {
					this.iphoneOperatoriSshow = 3;
					this.$refs[formName].resetFields(); //恢复表单及数据初始化状态
				} else {
					console.log('error submit!!');
					return false;
