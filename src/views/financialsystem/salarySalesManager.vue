<template>
    <div class="app-container calendar-list-container">
       ...
       <el-form :inline="true" :model="optionParams" ref="optionParams" class="demo-form-inline" @keyup.enter.native="getApplyList('isseach')">
            ...
            <el-form-item label="营业部经理" prop="areaId">
                <el-select v-model="optionParams.adminId" placeholder="请选择营业部经理" :disabled="!optionParams.shopId">
        ...
        <div class="filter-container">
            <!--<div class="tableheader">营业部经理</div>-->
            <div class="tablelayer">
                <el-table :data="dataList" v-loading="listLoading" @current-change="handleCurrentChange" border fit highlight-current-row style="width: 100%">
                    ...
                    <el-table-column align="center" label="营业部当月总业绩">
                        <template scope="scope">
                    ...
                    <el-table-column align="center" label="滚动7+逾期率系数">
                        <template scope="scope">
                            <span>{{scope.row.sevenOverAlRate}}</span>
                        </template>
                    </el-table-column>
                    <el-table-column align="center" label="人均达标津贴">
                        <template scope="scope">
                            <span>{{scope.row.avgAllowance}}</span>
                        </template>
                    </el-table-column>
                    <el-table-column align="center" label="营业部业绩提成">
                        <template scope="scope">
                            <span>{{scope.row.deptAllowance}}</span>
                        </template>
                    </el-table-column>
                    <el-table-column align="center" label="当月个人业绩提成">
                        <template scope="scope">
                            <span>{{scope.row.totalAllowance}}</span>
                        </template>
                    </el-table-column>
                    <el-table-column align="center" label="实际人数">
                        <template scope="scope">
                            <span>{{scope.row.yybPersonNum}}</span>
                        </template>
                    </el-table-column>
...
export default {
    ...
    methods: {
         // 下载
        mingdao() {
            downloadExecl({ url: 'export/exportofficerSalary', input: this.optionParams, method: 'get' })
        },
         //时间
        changeTime (value) {
        ...
        getApplyList(isSeach) {
            ...
            ajax({
                url: 'financeSalarySystem/officerSalary',
