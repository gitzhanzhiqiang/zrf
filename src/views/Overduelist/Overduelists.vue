            ...
            <el-table :data="dataList" v-loading="listLoading" border fit highlight-current-row style="width: 100%">
                ...
                <el-table-column align="center" label="订单编号" width="180">
                 ...
                 <el-table-column align="center" label="逾期状态" width="180">
                    <template scope="scope">
                        <span v-if ='1<=scope.row.overdueDays && scope.row.overdueDays<=3'>门店催收</span>
                        <span v-if ='4<=scope.row.overdueDays && scope.row.overdueDays<=7'>总部电催</span>
                          <span v-if ='8<=scope.row.overdueDays && scope.row.overdueDays<=40'>门店法催</span>
                          <span v-if ='scope.row.overdueDays>40'>综合电催</span>
                    </template>
                </el-table-column>
                ...
                <el-table-column align="center" label="分配（总部）" fixed="right" width="100" v-if='fenxian == 1'>
                    <template scope="scope">
                        <el-button type="text" size="small" @click="isAlert(scope.row)">分配</el-button>
                    </template>
                </el-table-column>
                <el-table-column align="center" label="催收员（门店）" fixed="right" width="100" v-if='fenxian == 1'>
                    <template scope="scope">
                        <el-button type="text" size="small" @click="isAlert(scope.row, true)">分配</el-button>
                    </template>
                </el-table-column>
            </el-table>
        </div>
...
export default {
    ...
    methods: {
        download() {
            downloadExecl({ url: 'export/exportOverDueList', input: this.params, method: 'get' })
        },
        getTable(isseach) {
