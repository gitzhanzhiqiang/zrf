<template>
    <div>
        <el-upload class="upload-demo" action="" multiple v-loading="loading" :before-upload="beforeAvatarUpload" :on-preview="handlePreview" :on-remove="handleRemove" :on-success="handleSuccess" :onError="uploadError" :show-file-list="true" :file-list="fileList2" list-type="picture">
...
export default {
    ...
    methods: {
        ...
        beforeAvatarUpload(file) {
            ...
            // 上传
            let that = this;
            let name = 'zrf/infoimg/' + this.getNowDate() + '/' + this.timestamp() + parseInt(Math.random() * 10000 + this.fileList2.length) + this.userId + '.' + fileName[fileName.length - 1];
